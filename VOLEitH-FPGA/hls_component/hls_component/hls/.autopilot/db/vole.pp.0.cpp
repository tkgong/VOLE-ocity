...
void expand_seed(
    IV &iv,
    hls::stream<ap_uint<128>> &seed_strm,
    hls::stream<ap_uint<256>> r_strm[1<<2]
){
#pragma HLS INLINE OFF
 const int N = 1<<2;
    const int STEP = (40273 + 256 -1)/ 256;

    VITIS_LOOP_16_1: for(int t = 0; t < 64; t++){
        ap_uint<128> sd[N];
...
#pragma HLS PIPELINE II=1
 VITIS_LOOP_26_2: for(int i = 0; i < N; i++){
                ap_uint<128> r0,r1;
                IV cur_iv = iv + 2*s;
...
void build_VOLE(
    hls::stream<ap_uint<256>> r_strm[1<<2],
    hls::stream<ap_uint<256>> &u_strm,
    hls::stream<ap_uint<256>> v_strm[64][2]
){
    const int N = 1<<2;
    const int STEP = (40273 +256 -1)/256;
#pragma HLS INLINE OFF
 PROCESS_BATCHES: for(int t = 0; t < 64; t++){
        PROCESS_CHUNKS: for(int s = 0; s < STEP; s++){
#pragma HLS PIPELINE II=1

 ap_uint<256> r_t[2 +1][N];
            ap_uint<256> v[2];
#pragma HLS ARRAY_PARTITION variable=r_t complete dim=0
#pragma HLS ARRAY_PARTITION variable=v complete dim=0

 INIT_V: for(int level = 0; level < 2; level++){
#pragma HLS UNROLL
 v[level] = 0;
            }
...
#pragma HLS UNROLL
 r_t[0][i] = r_strm[i].read();
            ...
            BUILD_LEVELS: for(int level = 0; level < 2; level++){
            ...
            WRITE_V: for(int level = 0; level < 2; level++){
#pragma HLS UNROLL
 v_strm[t][level].write(v[level]);
            }

            if(t == 0){
                u_strm.write(r_t[2][0]);
            }
        }
    }
}
...
#pragma HLS BIND_STORAGE variable=V type=ram_2p impl=URAM
 ...
 const int STEP = (40273 +256 -1)/256;
    VITIS_LOOP_100_1: for(int s=0; s< STEP; s++){
        ap_uint<256> u_t = u_strm.read();
#pragma HLS ARRAY_PARTITION variable=u type=cyclic factor=2
 UNPACK_U: for(int b = 0; b< 256; b++){
#pragma HLS UNROLL factor=2
 int idx = s*256 +b;
        ...
        ap_uint<256> v_t[64][2];
#pragma HLS ARRAY_PARTITION variable=v_t complete dim = 0
 READ_V:for(int i = 0; i < 64; i++){
#pragma HLS UNROLL
 VITIS_LOOP_114_2: for(int j=0; j<2;j++){
#pragma HLS UNROLL
...
#pragma HLS UNROLL factor=2
 ap_uint<128> v_pack=0;
            VITIS_LOOP_122_3: for(int i = 0; i<64;i++){
#pragma HLS UNROLL
 VITIS_LOOP_124_4: for(int j = 0; j< 2;j++){
#pragma HLS UNROLL
 int idx = i*2 +j;
...
#pragma HLS DATAFLOW
 const int N = 1<<2;
    hls::stream<ap_uint<256>> r_strm[N];

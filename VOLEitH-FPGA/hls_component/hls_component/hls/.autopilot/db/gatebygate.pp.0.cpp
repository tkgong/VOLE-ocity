...
# 5 "gatebygate.cpp" 2
void gf128_clmul(ap_uint<128> a, ap_uint<128> b, ap_uint<2*128> &result) {
...
#pragma HLS UNROLL factor=8
 if(b[i]) {
            result ^= ((ap_uint<2*128>)a) << i;
        }
    }
}
ap_uint<128> gf128_reduce(ap_uint<2*128> product) {
#pragma HLS INLINE

 ap_uint<128> low = product.range(128 -1, 0);
    ap_uint<128> high = product.range(2*128 -1, 128);


    ap_uint<128 +7> r1 = ((ap_uint<128 +7>)high) << 7;
    ap_uint<128 +7> r2 = ((ap_uint<128 +7>)high) << 2;
    ap_uint<128 +7> r3 = ((ap_uint<128 +7>)high) << 1;
    ap_uint<128 +7> r4 = high;

    ap_uint<128 +7> temp = r1 ^ r2 ^ r3 ^ r4;


    ap_uint<128> mid = temp.range(128 -1, 0);
    ap_uint<7> overflow = temp.range(128 +6, 128);

    ap_uint<128> o1 = ((ap_uint<128>)overflow) << 7;
    ap_uint<128> o2 = ((ap_uint<128>)overflow) << 2;
    ap_uint<128> o3 = ((ap_uint<128>)overflow) << 1;
    ap_uint<128> o4 = overflow;

    return low ^ mid ^ o1 ^ o2 ^ o3 ^ o4;
}

ap_uint<128> gf128_multiply(ap_uint<128> a, ap_uint<128> b) {
#pragma HLS INLINE off
#pragma HLS PIPELINE

 ap_uint<2*128> product;
    gf128_clmul(a, b, product);
    return gf128_reduce(product);
}

void combineVOLEs(ap_uint<128> vals[272], ap_uint<128> &result){
#pragma HLS INLINE OFF
 ap_uint<128> acc = 0;
    ap_uint<128> power = 1;

    VITIS_LOOP_57_1: for(int i = 0; i < 272; i++){
#pragma HLS PIPELINE OFF
 acc ^= gf128_multiply(vals[i], power);
        power = gf128_multiply(power, 2);
    }
    result = acc;
}
void compute_mask(
    ap_uint<1> u[386045],
    ap_uint<128> V[386045],
    ap_uint<128> &u_mask,
    ap_uint<128> &v_mask
){
...
#pragma HLS INLINE off
 ap_uint<128> ur[272];
    ap_uint<128> Vr[272];
    VITIS_LOOP_76_1: for(int i = 0; i < 272; i++){
...
void EvalCircuit(
    ...
    hls::stream<ap_uint<2*128>> &a0_strm,
    hls::stream<ap_uint<128>> &a1_strm
){
#pragma HLS INLINE OFF
 ap_uint<1> extended_witness[386045];

#pragma HLS BIND_STORAGE variable=u type=ram_2p impl=bram
#pragma HLS BIND_STORAGE variable=V type=ram_2p impl=uram
 VITIS_LOOP_100_1: for(int i = 0; i < 1601;i++){
#pragma HLS PIPELINE II=1
 extended_witness[i] = witness.read();
  ...
  VITIS_LOOP_107_2: for(int i = 0; i < 192086; i++){
#pragma HLS PIPELINE II=3
 GateInfo gate=circuit.read();
    ...
    if(gate.is_and){
        ...
        ap_uint<128> a1 = extended_witness[in1] * V[in2] + extended_witness[in2] * V[in1] - V[out];
        a0_strm.write(a0);
        a1_strm.write(a1);
    } else {
        extended_witness[out] = extended_witness[in1] ^ extended_witness[in2];
...
void ToField(
    hls::stream<ap_uint<2*128>> &a0_strm,
    hls::stream<ap_uint<128>> &a1_strm,
    hls::stream<ap_uint<2*128>> &a_strm
){
#pragma HLS INLINE OFF
 VITIS_LOOP_138_1: for(int i = 0; i < 38400; i++){
#pragma HLS PIPELINE II=1
 ap_uint<128> a1 = a1_strm.read();
        ap_uint<2*128> a0 = a0_strm.read();
        ap_uint<2*128> a;
...
#pragma HLS INLINE off
 ...
 ap_uint<128> agg_a0 = 0;
    ap_uint<128> agg_a1 = 0;

    VITIS_LOOP_163_1: for(int i = 0; i < 38400; i++){
#pragma HLS PIPELINE II=5


 ap_uint<2*128> a = a_strm.read();
        ap_uint<128> chi = ch2_strm.read();
        proof_strm.write(chi);

        ap_uint<128> a0 = a.range(128 -1, 0);
        ap_uint<128> a1 = a.range(2*128 -1, 128);
...
#pragma HLS BIND_STORAGE variable=V type=ram_2p impl=URAM
 ...
 hls::stream<ap_uint<128*2>> a0_strm;
    hls::stream<ap_uint<128>> a1_strm;
    hls::stream<ap_uint<1>> d_strm_cp;
    hls::stream<ap_uint<128>> ch2_strm;
    hls::stream<ap_uint<128*2>> a_strm;
    ap_uint<128> u_mask;
    ap_uint<128> v_mask;

...
//#pragma HLS bind_op variable=blkNumReg op=div impl=DivnS //latency=20
...
//#pragma HLS bind_op variable=blkNumReg op=div impl=DivnS //latency=200
...
// ===== SQUEEZE PHASE (CORRECTED) =====
        ap_uint<128> numOutputChunks = (outLen + 15) >> 4;
        ap_uint<128> bytesLeft = outLen;
        ap_uint<5> wordIdx = 0;  // Track 64-bit word position (0-16)

        for (ap_uint<128> chunk = 0; chunk < numOutputChunks; chunk++) {
            // *** FIX: Removed II=1, this loop calls KECCAK_f ***
            #pragma HLS pipeline OFF
            #pragma HLS loop_tripcount min=1 max=16 avg=8

            ap_uint<128> output;

            // --- Read the first 64 bits (LOWER part) ---
            if (wordIdx >= numMsgWord) { // numMsgWord is 17
                KECCAK_f(stateArray.M);
                wordIdx = 0;
            }
            ...
            // --- Read the second 64 bits (UPPER part) ---
            ...
            // (Your masking logic for the last chunk is fine)
            if (bytesLeft < 16) {
        ...
        // Read the next set of control flags
        msgEndFlag = endMsgLenStrm.read();
        outEndFlag = endOutLenStrm.read();
        
        // *** BUG FIX 2: Removed endDigestStrm.write(true) from inside the loop ***
    }

    // *** BUG FIX 2: Moved here. Signals "all messages are done" ***
    endDigestStrm.write(true);
}
} // namespace security
} // namespace xf
